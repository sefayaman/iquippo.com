        ...
        AuctionSvc.getAuctionDateData(filter)
          .then(function(res) {
            ...
            $scope.contactNumber = res.items[0].contactNumber;
             ...
             filter={};
             filter.auctionId=res.items[0].auctionId;
...
function fetchAsset(assetId,lotNumber,displayBid){
    ...
    .catch(function(err){
     if(err) throw err;
            ...
            $scope.formats = ['yyyy', 'dd-MMMM-yyyy', 'yyyy/MM/dd', 'dd.MM.yyyy', 'shortDate']; $scope.format = $scope.formats[0];
