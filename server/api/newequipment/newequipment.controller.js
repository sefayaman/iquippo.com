...
var Seq = require('seq');
var NewEquipment = require('./newequipment.model.js');
...
exports.get = function(req, res) {
 ...
 /* if (queryParam.category)
  ...
  var query = NewEquipment.find(filter);
...
exports.create = function(req, res) {console.log("req.body=",req.body);
  NewEquipment.create(req.body, function(err, respo) {
...
exports.getOnFilter = function(req, res) {
  ...
  if(req.body.pagination){
    Utility.paginatedResult(req,res,NewEquipment,filter,{});
    return;    
  }
  var sortObj = {}; 
  if(req.body.sort)
  ...
  var query = NewEquipment.find(filter).sort(sortObj);
  Seq()
  .par(function(){
    var self = this;
    NewEquipment.count(filter,function(err, counts){
...
exports.update = function(req, res) {
  ...
  NewEquipment.findById(req.params.id, function (err, inputReq) {
    if (err) { return handleError(res, err); }
    if(!inputReq) { return res.status(404).send('Not Found'); }
    NewEquipment.update({_id:req.params.id},{$set:req.body},function(err){
        if (err) { return handleError(res, err); }
