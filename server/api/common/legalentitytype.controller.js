...
var Model = require('./legalentitytype.model');
...
exports.get = function(req, res) {
  ...
  if (queryParam.legalType)
    filter.legalType = queryParam.legalType;
...
/*exports.search = function(req, res) {
  ...
  if (body.legalType)
    filter.legalType = body.legalType;
...
exports.create = function(req, res, next) {
  _getRecord(req.body, function(err, result) {
    ...
    if (result.length > 0)
      return next(new ApiError(409, "Legal type already exits!!!"));
    else
      create();
  });
  ...
  function create() {
    ...
    model.save(function(err, st) {
      ...
      return res.status(200).json({
        message: "Legal type saved sucessfully"
      });
    });
  }
};
...
function _getRecord(data, cb) {
  var filter = {};
  filter.legalType = data.legalType;
  ...
  Model.findById(req.params.id, function(err, oneRow) {
    ...
    oneRow.remove(function(err) {
      ...
      return res.status(204).send({
        message: "Legal type deleted sucessfully!!!"
      });
    });
  });
};
