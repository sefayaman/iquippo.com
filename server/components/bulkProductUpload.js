...
bulkProductUpload.updateProductMaster = function(req,res){
  Model.find({'group.name':"Other",'category.name':"Other",'brand.name':"Other",name:"Other"},function(err,models){
    if(err){return res.status(500).send(err); }
    if(models.length == 0)
      return res.status(404).send("Other model not found");
    if(models.length > 1)
      return res.status(404).send("More than one model found");
    var query = Product.find({$or:[{"brand.name":/^Other/i},{"category.name":/^Other/i},{"model.name":/^Other/i}],deleted:false});
...
function updateProducts(req,res,products){
  if(req.counter < req.total){
    ...
    Product.update({_id:id},{$set:prd},function(err,data){
